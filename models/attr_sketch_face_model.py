...
class attrsketchfaceModel():
    ...
    def backward_D2(self):
        ...
        self.pred_fake = self.netD2.forward(fake_AB.detach()[:64,:4,:4,:4].cuda())
    ...
    def backward_G2(self):
        ...
        fake_AB = torch.cat((self.recon[0], self.recon2), 1)
        self.pred_fake = self.netD2.forward(fake_AB[:,:4,:,:])
        self.loss_G2_GAN = self.criterionGAN(self.pred_fake, True)

        self.real_resize = Variable(util.vgg_initial(self.real_sketch)).cuda()
        self.perp_real = self.netP.forward(self.real_resize.cuda())[0]
        self.fake_resize = Variable(util.vgg_initial(self.recon2)).cuda()
        self.perp_fake = self.netP.forward(self.fake_resize.cuda())[0]
        self.loss_percp = self.criterionL1(self.perp_fake, self.perp_real) * 10

        print(self.recon2.shape)
        print(self.real_sketch.shape)
        self.recon2 = self.recon2[:, :3,:, :]
    ...
    def backward_G3(self):
        ...
        self.real_resize = Variable(util.vgg_initial(self.real_image)).cuda()
        self.perp_real = self.netP.forward(self.real_resize)[0].cuda()
        self.fake_resize = Variable(util.vgg_initial(self.recon_img)).cuda()
        self.perp_fake = self.netP.forward(self.fake_resize)[0].cuda()
    ...
    def get_current_errors(self, stage):
        if stage == 1:
            return OrderedDict([
                                ('G_GAN', 0),
                                ('G_Gaussian', self.Gaussian_loss.item()),
                                ('KLD_latent', self.KLloss_latent.item()),
                                ('KLD_z', self.KLloss_z.item()),
                                ('D_real', 0),
                                ('D_wrong', 0),
                                ('D_fake', 0),
                                ])
        elif stage == 2:
            return OrderedDict([('G_GAN', self.loss_G2_GAN.item()),
                                ('G_Gaussian', self.loss_G2_L1.item()),
                                ('perceptual_loss', self.loss_percp.item()),
                                ('KLD_latent', 0),
                                ('KLD_z', 0),
                                ('D_real', self.loss_D2_real.item()),
                                ('D_wrong', 0),
                                ('D_fake', self.loss_D2_fake.item()),
                                ])
        elif stage == 3:
            return OrderedDict([('G_GAN', self.loss_G3_GAN.item()),
                                ('G_Gaussian', self.loss_G3_L1.item()),
                                ('perceptual_loss', self.loss_percp.item()),
                                ('KLD_latent', 0),
                                ('KLD_z', 0),
                                ('D_real', self.loss_D3_real.item()),
                                ('D_wrong', 0),
                                ('D_fake', self.loss_D3_fake.item()),
                                ])
    ...
    def load_network(self, network, network_label, epoch_label):
        ...
        save_path = os.path.join("C:\\Users\\sft\\Desktop\\大四下\\数字图像处理\\code\\Attribute2Sketch2Face\\checkpoints\\celeba_attr2sketch2face", save_filename)
        print(save_path)
