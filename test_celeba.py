...
face_dataset = CreateDataLoader(opt, csv_fileA='./dataset/raw_images/CelebA_Anno/list_attr_celeba.csv',root_dirA='./dataset/CelebA/Img/test_sft',
                                    csv_fileB='./dataset/raw_images/CelebA_Anno/list_attr_celeba.csv', root_dirB='./dataset/CelebA/Img/test_sft')
...
for i in range(test_num):
    ...
    if i >= opt.how_many:
        break
    # print(face_dataset[idx])
    a = face_dataset[idx]
    print(a['sketch'].shape)

    # b = np.random.normal(size=a['sketch'].shape)/2
    # print(b)
    # a['sketch'] = a['sketch'] + b
    # print(a)

    # b = np.random.normal(size=a['image'].shape)/2
    # a['image'] = a['image'] + b
    # break
    model.set_input(a)
    model.test()
    ...
    print("process images... %s %s".format(img_pathA , img_pathB) )
    print(img_pathA)
    print(img_pathB)
    visualizer.save_inception_images(webpage, visuals, img_pathA)
    # visualizer.save_images(webpage, visuals, "./results/celeba_attr_sketch_face\\test_30\\image\\%s".format(i))
